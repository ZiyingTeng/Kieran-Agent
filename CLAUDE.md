...
- **`session_manager.py`** — Redis-backed stateless session management. TTL-LRU cache for in-process Agent objects, Redis for session metadata + RedisMemory. Enables horizontal scaling across Gunicorn workers.
...
- **`user_profile_manager_v2.py`** — Structured user profiling with 5 semantic tables. Uses Mem0 for fact extraction and relay-gemma4-remote for qualitative synthesis (not raw fact copying). Profiles stored as JSON in `user_profiles/`. Triggered every 5 rounds via `summarize_and_save()`. **Currently disabled** (injection commented out in `llm_service.py`, trigger commented out in `app.py`) pending architectural improvement: injection should happen per-request rather than only at agent-creation time, so current-session profiles take effect immediately.
- **`heartbeat_scheduler.py`** / **`heartbeat_greeting_generator.py`** — Proactive engagement system. APScheduler with Redis leader election, probability-based greeting, story-driven LLM prompts.
...
1. **Short-term** (InMemoryMemory) — current session chat messages, lives in the Agent object. Trimmed to last 20 messages per request (`MAX_PRIVATE_CHAT_HISTORY`).
2. **Medium-term** (RedisMemory) — cross-request session state, survives worker restarts
3. **Long-term** (Mem0 + PostgreSQL) — persistent user knowledge and profile data. Protected by LTM Noise Control (see below).
...
User message -> WebSocket/HTTP -> Session retrieval (Redis + LRU cache) -> Agent init or retrieval (`llm_service.create_agent`) -> Response generation -> Memory updates (all 3 tiers) -> Response to user
...
- **Restart:** `pkill -f 'uvicorn app:app' && cd /data/test/Chatjoy2 && nohup venv/bin/python3 venv/bin/uvicorn app:app --host 0.0.0.0 --port 8002 --workers 1 >> logs/app.log 2>&1 &`
